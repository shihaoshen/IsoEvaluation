...
def glmer(input_inc, input_tot, input_group):
	...
	ofile2.close();
	subprocess.call("Rscript glmer.R ", shell=True)
	ifile2=open('glmer_output.txt');
...
while iline:
	...
	if len(elements)>0:
		...
		if countavg >= count_cutoff:
			...
			if len(sys.argv)>2:
				if sys.argv[2]=='1':
					p = glmer(input_inc, input_tot, input_group);
		else:
